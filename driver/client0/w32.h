...
// MY TYPE
/////////////////
// typedef struct EXECUTABLE_FILE_INFO {
// 	PVOID base_x; // Базовый адрес в проекции
// 	PVOID pImageBase; // Адрес не в проекции
// 	PVOID base_pmap; // Адрес в проекции
// 	DWORD g_e_lfanew; // смещение до заголовка
// 	DWORD dwSizeHeaders; // размер IMAGE_NT_HEADERS
// 	DWORD g_SizeOptHeader; // размер опционального заголовка
// 	PIMAGE_NT_HEADERS g_pImageNtHeader; // данные полученные из памяти после загрузки модуля
// 	ulong dep_imp; // количество используемых библиотек
// 	ULONG g_countImportApi; // размер массива импорта
// 	PIMPORT_API g_pImportApi; // массив импорта (по именам)
// 	ULONG g_countExportApi; // размер массива экспорта
// 	PEXPORT_API g_pExportApi; // массив экспорта
// 	WORD g_SectionsCount; // количество секций
// 	PIMAGE_SECTION_HEADER* g_pSectionHeaders; // массив указателей на секции
// 	unsigned char error_flag; // флаг инициализации
// 	PMDL pmdl;
// } EFI_X, *PEFI;
...
typedef struct EXE_FILE_
{
	DWORD m_dwSizeHeaders;
	LONG m_e_lfanew;
	DWORD m_SizeOptHeader;
	PVOID m_base_x;
...
EXTERN_C int InitBase(PEXE_FILE, PVOID);
EXTERN_C int InitExport(PEXE_FILE);
EXTERN_C int InitImport(PEXE_FILE);
EXTERN_C int InitSection(PEXE_FILE);
EXTERN_C int BasicValidation(PVOID);
EXTERN_C void ExeFileFree(PEXE_FILE);
EXTERN_C ULONG get_count_of_dependency_libs(PEXE_FILE);

EXTERN_C PIMAGE_SECTION_HEADER GetSectionByAddr(PEXE_FILE pexefile, PVOID p);
EXTERN_C PVOID GetExportedFn(PEXE_FILE, PCHAR); //const PEXE_FILE pexefile, const char* szName
EXTERN_C PVOID GetExportedFnByOrd(PEXE_FILE, WORD);
EXTERN_C const PCHAR GetExportedFnByAddr(PEXE_FILE, PVOID);
EXTERN_C PVOID GetImportedFn(PEXE_FILE, PCHAR);
EXTERN_C const PCHAR GetImportedFnByAddr(PEXE_FILE, PVOID);
EXTERN_C PEXPORT_FN GetExportedFnInfo(PEXE_FILE, PCHAR);
EXTERN_C PIMPORT_FN GetImportedFnInfo(PEXE_FILE, PCHAR);
EXTERN_C PIMPORT_FN GetImportedFnInfoByLibAndName(PEXE_FILE, PCHAR, PCHAR);